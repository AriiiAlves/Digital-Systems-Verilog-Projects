...
module deslocador_bidirecional(input serial_input, clk, right, rst, output [4:0] Q);
    ...
    assign D[0] = (serial_input && right) || (~right && Q[1]);
    assign D[1] = (Q[0] && right) || (~right && Q[2]);
    assign D[2] = (Q[1] && right) || (~right && Q[3]);
    assign D[3] = (Q[2] && right) || (~right && Q[4]);
    assign D[4] = (Q[3] && right) || (~right && serial_input);
    ...
    initial begin
        $display("[Desl Bid] Circuito Deslocador Bidirecional carregado.");
    end

    always @(posedge(clk) or posedge(rst))
    begin
        $display("[Desl Bid] Valor atual: %b", Q);
    end
endmodule
